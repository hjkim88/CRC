...
demoAnalysis <- function(clinInfoPath_640 = "//isilon.c2b2.columbia.edu/ifs/archive/shares/bisr/Parvathi_Myer/data/coadread_tcga_clinical_data.tsv",
  ...
  ### remove rows with MSI_RACE_Status == NA
  plot_df2 <- plot_df
  if(length(which(is.na(plot_df2$MSI_RACE_Status))) > 0) {
    plot_df2 <- plot_df2[-which(is.na(plot_df2$MSI_RACE_Status)),]
  }
  ...
  ### set parameters for the circular plot (stage)
  group <- unique(plot_df2$MSI_RACE_Status)
  separator_size <- 3
  ...
  for(i in 1:length(group)) {
    ...
    cdf$Group[((i-1)*(length(tumor_stage_list)+separator_size)+1):((i-1)*(length(tumor_stage_list)+separator_size)+length(tumor_stage_list)+separator_size)] <- group[i]
    for(j in 1:length(tumor_stage_list)) {
      cdf$Number[(i-1)*(length(tumor_stage_list)+separator_size)+j] <- length(intersect(which(plot_df2$MSI_RACE_Status == group[i]), which(plot_df2$Tumor_Stage == tumor_stage_list[j])))
    }
  }
  cdf$ID <- 1:nrow(cdf)
  ...
  for(i in 1:length(group)) {
    ...
    cdf$Group[((i-1)*(length(gender_list)+separator_size)+1):((i-1)*(length(gender_list)+separator_size)+length(gender_list)+separator_size)] <- group[i]
    for(j in 1:length(gender_list)) {
      cdf$Number[(i-1)*(length(gender_list)+separator_size)+j] <- length(intersect(which(plot_df2$MSI_RACE_Status == group[i]), which(plot_df2$Gender == gender_list[j])))
    }
  }
  cdf$ID <- 1:nrow(cdf)
  ...
  fit <- survfit(Surv(Survival, Status) ~ MSI_RACE_Status, data = plot_df2)
  
  p6 <- ggsurvplot(
    fit,
    data = plot_df2,
    title = "Survival Differences in Various MSI/Age Status",
    legend.labs = levels(as.factor(plot_df2$MSI_RACE_Status)),
    risk.table = TRUE,
    tables.col = "strata",
    pval = TRUE,
    conf.int = TRUE,
  ...
  ### beeswarm plot
  p7 <- ggplot(plot_df2, aes(x=MSI_RACE_Status, y=Survival)) +
    theme_classic(base_size = 16) +
    geom_boxplot() +
    geom_beeswarm(aes(color=MSI_RACE_Status), na.rm = TRUE) +
    stat_compare_means() +
    labs(x = "", y = "Survival (Months)") +
    theme(legend.position="top")
