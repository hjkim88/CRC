...
#
#   Instruction
#               1. Source("RNASEQAnalysis3.R")
#               2. Run the function "rnaseq_parvathi3" - specify raw count path, clinical info file paths and the output directory
#               3. The results will be generated under the output directory
#
#   Example
#               > source("The_directory_of_RNASEQAnalysis3.R/RNASEQAnalysis3.R")
#               > rnaseq_parvathi3(rCntPath = "./data/rnaseq/raw_count_tcga_coad_read.rda",
...
#                                  padj_thres = 0.05,
#                                  outputDir="./results/rnaseq/preprocessed_raw_counts/")
###

rnaseq_parvathi3 <- function(rCntPath = "//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/GTEx/RDA_Files/TCGA_RAW_COUNTS.rda",
  ...
  rCnt <- raw_count_tcga_coad_read[,which(sample_info_tcga_coad_read$`Case ID` %in% clinicalInfo_640$`Patient ID`[union(which(clinicalInfo_640$MSI_AGE_Status == "MSI-H_Young"),which(clinicalInfo_640$MSI_AGE_Status == "MSI-H_Old"))])]
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(rCnt),"Case ID"],"MSI_AGE_Status"]
  ...
  par(mfrow=c(1,2))
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(normCnt),"Case ID"],"MSI_AGE_Status"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  ...
  ### extract raw counts of samples of our interest and run DE analysis
  rCnt <- raw_count_tcga_coad_read[,which(sample_info_tcga_coad_read$`Case ID` %in% clinicalInfo_640$`Patient ID`[union(which(clinicalInfo_640$MSI_AGE_Status == "MSS_Young"),which(clinicalInfo_640$MSI_AGE_Status == "MSS_Old"))])]
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(rCnt),"Case ID"],"MSI_AGE_Status"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  deresult <- deseqWithComparisons(rCnt = rCnt, grp = grp,
                                   exp_class = "Young", ctrl_class = "Old",
  ...
  ### extract raw counts of samples of our interest and run DE analysis
  rCnt <- raw_count_tcga_coad_read[,which(sample_info_tcga_coad_read$`Case ID` %in% clinicalInfo_640$`Patient ID`[union(which(clinicalInfo_640$MSI_RACE_Status1 == "MSI-H_AA"),which(clinicalInfo_640$MSI_RACE_Status1 == "MSI-H_CC"))])]
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(rCnt),"Case ID"],"MSI_RACE_Status1"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  deresult <- deseqWithComparisons(rCnt = rCnt, grp = grp,
  ...
  par(mfrow=c(1,2))
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(normCnt),"Case ID"],"MSI_RACE_Status1"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  if(length(which(is.na(grp))) > 0) {
    grp[which(is.na(grp))] <- "NA"
  }
  ...
  ### extract raw counts of samples of our interest and run DE analysis
  rCnt <- raw_count_tcga_coad_read[,which(sample_info_tcga_coad_read$`Case ID` %in% clinicalInfo_640$`Patient ID`[union(which(clinicalInfo_640$MSI_RACE_Status1 == "MSS_AA"),which(clinicalInfo_640$MSI_RACE_Status1 == "MSS_CC"))])]
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(rCnt),"Case ID"],"MSI_RACE_Status1"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  deresult <- deseqWithComparisons(rCnt = rCnt, grp = grp,
                                   exp_class = "AA", ctrl_class = "CC",
  ...
  ### extract raw counts of samples of our interest and run DE analysis
  rCnt <- raw_count_tcga_coad_read[,which(sample_info_tcga_coad_read$`Case ID` %in% clinicalInfo_640$`Patient ID`[union(which(clinicalInfo_640$MSI_RACE_Status2 == "MSI-H_AA"),which(clinicalInfo_640$MSI_RACE_Status2 == "MSI-H_CC"))])]
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(rCnt),"Case ID"],"MSI_RACE_Status2"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  deresult <- deseqWithComparisons(rCnt = rCnt, grp = grp,
                                   exp_class = "AA", ctrl_class = "CC",
                                   bat_eff = NULL, thresh = 1)
  ...
  ### extract raw counts of samples of our interest and run DE analysis
  rCnt <- raw_count_tcga_coad_read[,which(sample_info_tcga_coad_read$`Case ID` %in% clinicalInfo_640$`Patient ID`[union(which(clinicalInfo_640$MSI_RACE_Status2 == "MSS_AA"),which(clinicalInfo_640$MSI_RACE_Status2 == "MSS_CC"))])]
  grp <- clinicalInfo_640[sample_info_tcga_coad_read[colnames(rCnt),"Case ID"],"MSI_RACE_Status2"]
  grp <- sapply(grp, function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][2], USE.NAMES = FALSE)
  deresult <- deseqWithComparisons(rCnt = rCnt, grp = grp,
                                   exp_class = "AA", ctrl_class = "CC",
  ...
  # 
  # ### extract raw counts of samples of our interest and run DE analysis
  # rCnt <- raw_count_tcga_paad[,union(which(sample_info_tcga_paad$race == "black or african american"), which(sample_info_tcga_paad$race == "white"))]
  # grp <- sample_info_tcga_paad[colnames(rCnt),"race"]
  # grp <- sapply(grp, function(x) {
  #   if(x == "black or african american")
  #     return("AA")
  #   else if(x == "white")
  ...
  # dev.off()
  # 
  # ### test with other TCGA data -LUAD
  # load("./data/rnaseq/raw_count_tcga_luad.rda")
  # 
  # ### extract raw counts of samples of our interest and run DE analysis
  # rCnt <- raw_count_tcga_luad[,union(which(sample_info_tcga_luad$race == "black or african american"), which(sample_info_tcga_luad$race == "white"))]
  # grp <- sample_info_tcga_luad[colnames(rCnt),"race"]
  # grp <- sapply(grp, function(x) {
  #   if(x == "black or african american")
  #     return("AA")
  #   else if(x == "white")
  ...
  # dev.off()
  # 
  # ### test with other TCGA data -BRCA
  # load("./data/rnaseq/raw_count_tcga_brca.rda")
  # 
  # ### extract raw counts of samples of our interest and run DE analysis
  # rCnt <- raw_count_tcga_brca[,union(which(sample_info_tcga_brca$race == "black or african american"), which(sample_info_tcga_brca$race == "white"))]
  # grp <- sample_info_tcga_brca[colnames(rCnt),"race"]
  # grp <- sapply(grp, function(x) {
  #   if(x == "black or african american")
  #     return("AA")
  #   else if(x == "white")
